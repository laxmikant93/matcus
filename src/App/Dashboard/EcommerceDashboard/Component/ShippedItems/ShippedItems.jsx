...
import { customerOrderDetail, getItemSuborders, getShipItems,updateSubOrderFailAndDelivered,getDeliverItems} from '../../../../../store/actions/ecommerce/action/cartOrder';
...
import { showSuccessPopup } from './../../../../../store/actions/successmessagepopup/index';
const ShippedItems = ({ orderId }) => {
  ...
  const { getShipItemsSuccess, getShipItemsData,updateDeliverFailedSubOrderLoaded,updateDeliverFailedSubOrderSuccess } = useSelector((state) => {
    return {
...
// console.log("all",deliveredData)
  ...
  const handleInput=(e)=>{
      ...
      if(e.target.value==="Delivered"){
        ...
        dispatch(updateSubOrderFailAndDelivered(orderId,{data:data,suborderList:deliveredData},"Delivered"))
        /* setTimeout(()=>{
          dispatch(getShipItems(orderId));
    ...
    <div>
      <div className="gridListTable  ">
        <ul className="gridHeader ship-item-table-header">
          ...
          <li className="col col-2 item-lable-heading text-center ">PRICE</li>
          <li className="col col-2 item-lable-heading text-center ">QUANTITY</li>
          <li className="col col-2 item-lable-heading text-center ">STATUS</li>
          <li className="col col-2 item-lable-heading text-center " >TRACK</li>
        </ul>
        <div className="gridBody shipped-item-table-wrapper">
          {
            getShipItemsData && getShipItemsData.map((item) => (
              <div className="gridRow  shipped-item-table-row " key={''}>
                <ul className="topInfo">
                  ...
                  <li className="col col-2 text-center " data-head="PRICE">
                  ...
                  <li className="col col-2 text-center" data-head="QUANTITY">
                    <div className='order-list-th-wrapper'>
                      <p className='text-xxs base w-400'>{item?.data?.length}</p>
                    </div>
                  </li>
                  <li className="col col-2 text-center" data-head="STATUS">
                  ...
                  <li className="col col-2 text-center" data-head="TRACK" >
                    <div className='text-center view-tracking-details-wrapper ' onClick={() => { onOpenSelfShipping();onClickSendData(item); }}>
                    ...
                    </div>
                    {
                      ...
                      <TrackingDetailPopup openref={openselfshipping} onclose={onCloseSelfShipping} shippingPartner={item?.data[0]?.shipping_partner} trackingId={item?.data[0]?.shipping_tracking_id}
                       trackingUrl={item?.data[0]?.tracking_url}
                       propData={propData}
                       />
