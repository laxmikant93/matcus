...
export default function InstituteMenuListItem() {
  ...
  const handleShowMenu = (index, link, name) => {
    if (link) {
      setActive(true)
      history(link)
    } else {
            ...
            <li className={`menuListItem`} key={key}>
              {
                item.subMenuItem && item.subMenuItem.length >= 1
                  ...
                  item.menuName === "Templates" && !user.user_institute_isOld ? (
                    <button
                      // to={item.menuLink}
                      className={pathname === item.menuLink && active ? "active" : ""}
                      onClick={() => handleShowMenu("clear", item.menuLink)}
                    >
                      <span className="iconCustom">
                        <i className={`menuIcon ${item.Icon}`}></i>
                      </span>
                                      ...
                                      <li className={`subMenuListItem`}>
                                        {
                                          subMenuItem2.subMenuItem && subMenuItem2.subMenuItem.length >= 1
                                            ...
                                            <>
                                              {
                                                subMenuItem2.menuName === "Home & Menu" && user.user_institute_isOld === false ? ("") : (
